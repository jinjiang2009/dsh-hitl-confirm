...
const CSS = [
  // Full-viewport dimmed backdrop + centered dialog card.
...
function isHitlQuestion(payload) {
  return Array.isArray(payload?.questions)
    && payload.questions.some((q) => q != null && typeof q.header === 'string' && q.header === HITL_HEADER)
}
...
function HitlModal(props) {
  ...
  const t = props.t
  // This plugin always asks exactly one branded question per request.
  ...
  const toggle = useCallback(function (label) {
    setError(null)
  ...
  const confirm = useCallback(async function () {
  ...
  }, [wait, question, selected, custom, t])
  ...
  const optionCards = options.map(function (option, index) {
    ...
    return createElement('button', {
      ...
      disabled: busy,
      onClick: function () { toggle(label) },
    },
      createElement('span', { className: 'hitl-option-key', 'aria-hidden': 'true' },
        String.fromCharCode(65 + index) /* A, B, C … */),
      createElement('span', { className: 'hitl-option-text' },
        createElement('span', { className: 'hitl-option-label' }, label),
        option.description != null
          ? createElement('span', { className: 'hitl-option-desc' }, String(option.description))
          : null,
      ),
    )
  })

  return createElement('div', { className: 'hitl-backdrop' },
    createElement('div', {
      ...
      createElement('div', { className: 'hitl-titlebar' },
        createElement('span', { className: 'hitl-pulse', 'aria-hidden': 'true' }),
        createElement('span', null, t('title')),
      ),
      createElement('div', { className: 'hitl-body' },
        createElement('div', { className: 'hitl-question' }, question.question),
        question.detail != null
          ? createElement('div', { className: 'hitl-context' },
              createElement('span', { className: 'hitl-context-title' }, t('context.title')),
              createElement('span', null, String(question.detail)),
            )
          : null,
        options.length > 0
          ? createElement('div', { className: 'hitl-options' }, optionCards)
          : null,
        createElement('div', { className: 'hitl-custom' },
          createElement('label', { className: 'hitl-custom-label', htmlFor: 'hitl-custom' }, t('custom.label')),
          createElement('textarea', {
            ...
            value: custom,
            disabled: busy,
            onChange: function (event) { setCustom(event.target.value); setError(null) },
          }),
        ),
        error != null ? createElement('div', { className: 'hitl-error' }, error) : null,
      ),
      createElement('div', { className: 'hitl-footer' },
        createElement('span', { className: 'hitl-hint' }, multi ? t('hint.multi') : t('hint.single')),
...
export function apply(ctx) {
  ...
  ctx.slots.inject('conversation.composer', function () {
    return ctx.slots.register({
      ...
      id: 'hitl-confirm',
      // Ascending chain order: -100 lands before the built-in question UI
      // (priority 0), so branded HITL dialogs are claimed here.
      priority: -100,
      select: selectHitl,
      locale: NS,
      label: 'HITL confirm',
