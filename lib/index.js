...
function questionId() {
	return "hitl:" + (globalThis.crypto?.randomUUID?.() ?? String(Math.random()).slice(2));
}
...
function buildQuestions(args) {
	return [{
		...
		header: HITL_HEADER,
		...args.context !== void 0 ? { detail: args.context } : {},
		options: args.options.map((option) => ({
			label: option.label,
			...option.description !== void 0 ? { description: option.description } : {}
		})),
		...args.multiSelect !== void 0 ? { multiSelect: args.multiSelect } : {}
	}];
}
...
function parseInvocation(raw) {
	...
	return {
		...
		options: options.length > 0 ? options : [{
			label: "Approve",
			description: "Agree and proceed with the proposed action."
		}, {
			label: "Reject",
			description: "Do not proceed; the agent should reconsider."
		}]
	};
}
function apply(ctx) {
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			return mapAnswer(await ctx.userQuestions.ask({
	...
	ctx.commands.register({
		...
		handler: async (invocation) => {
			...
			try {
				...
				return {
					kind: "error",
					text: "/hitl-confirm: " + (error instanceof Error ? error.message : String(error))
				};
			}
			try {
				const answer = (await ctx.userQuestions.ask({
					questions: buildQuestions(parsed),
					...invocation.agent !== void 0 ? { agent: invocation.agent } : {}
				})).answers[0];
				return {
					kind: "success",
					text: "Human confirmed: " + (answer.custom !== void 0 && answer.custom !== "" ? answer.custom : answer.selected.join(", "))
				};
			} catch (error) {
				return {
					kind: "error",
